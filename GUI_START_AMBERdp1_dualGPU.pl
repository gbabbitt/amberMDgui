...
print "\nDo you want an enriched feature vector for later machine learning ? (y or n)\n";
print "(non-enriched feature considers only atom fluctuations)\n";
print "(enriched feature considers atom fluctuations plus local shape change defined by\n";
print "motion correlations with adjacent neighboring residues)\n";
...
if($vector_enter eq 'y'){
    ...
    print "parsing atomcorr for corrALL_$fileIDq"."_$i.txt\n";
    # create ouput file 1 (1 residue distance)
    open (OUT1, ">"."corr_1res_$fileIDq"."_$i.txt"); 
    print OUT1 "Atom Atom ACorr_00005\n";
    # create ouput file 2 (3 residue distance)
    open (OUT2, ">"."corr_3res_$fileIDq"."_$i.txt");
    print OUT2 "Atom Atom ACorr_00005\n";
    # create ouput file 3 (5 residue distance)
    open (OUT3, ">"."corr_5res_$fileIDq"."_$i.txt");
    print OUT3 "Atom Atom ACorr_00005\n";
    # create ouput file 4 (10 residue distance)
    open (OUT4, ">"."corr_9res_$fileIDq"."_$i.txt");
    print OUT4 "Atom Atom ACorr_00005\n";
    # initiate 3 arrays
    #open each corrALL file
    open (IN, "<"."corrALL_$fileIDq"."_$i.txt");
    my @IN = <IN>;
    for (my $i = 0; $i < scalar @IN; $i++){
	 my $INrow = $IN[$i];
      my $INrow1 = $IN[$i+1];
...
print "for simple DNA-protein interaction analysis 'homology' will be loose\n";
$homology = "loose";
sleep(2);
...
open(IN1, "<"."DROIDSfluctuationAVGchain.txt") or die "could open output file\n";
@IN1 = <IN1>;
for (my $i = 0; $i < scalar @IN1; $i++){
    if ($i == 0){next;}
    print "creating file ./atomcorr/DROIDScorrelation_$i.txt\n";
    open(OUT, ">"."./atomcorr/DROIDScorrelation_$i.txt") or die "could not open output file\n";
    print OUT "sample\t"."pos_ref\t"."res_ref\t"."res_query\t"."atomnumber\t"."atomlabel\t"."flux_ref\t"."flux_query\t"."corr1_ref\t"."corr1_query\t"."corr3_ref\t"."corr3_query\t"."corr5_ref\t"."corr5_query\t"."corr9_ref\t"."corr9_query\n";
    open(IN2, "<"."./atomflux/DROIDSfluctuation_$i.txt") or die "could open not input file\n";
    @IN2 = <IN2>;
    for (my $ii = 0; $ii < scalar @IN2; $ii++){
        ...
        $corr_9res_ref = '';
        $corr_9res_query = '';
        # find matching correlations to alpha carbon atom of neighbors dist 1, 3, 5, 9 residues away
        open(IN3, "<"."corr_1res_$fileIDr"."_$sample.txt") or die "could open not input file\n";
            @IN3 = <IN3>;
            for (my $j = 0; $j < scalar @IN3; $j++){
            my $INrow = $IN3[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_1res_ref = $corr_test;}
            }
            close IN3;
        open(IN4, "<"."corr_1res_$fileIDq"."_$sample.txt") or die "could open not input file\n";
            @IN4 = <IN4>;
            for (my $j = 0; $j < scalar @IN4; $j++){
            my $INrow = $IN4[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_1res_query = $corr_test;}
            }
            close IN4;
        open(IN5, "<"."corr_3res_$fileIDr"."_$sample.txt") or die "could open not input file\n";
            @IN5 = <IN5>;
            for (my $j = 0; $j < scalar @IN5; $j++){
            my $INrow = $IN5[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_3res_ref = $corr_test;}
            }
            close IN5;
        open(IN6, "<"."corr_3res_$fileIDq"."_$sample.txt") or die "could open not input file\n";
            @IN6 = <IN6>;
            for (my $j = 0; $j < scalar @IN6; $j++){
            my $INrow = $IN6[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_3res_query = $corr_test;}
            }
            close IN6;
        open(IN7, "<"."corr_5res_$fileIDr"."_$sample.txt") or die "could open not input file\n";
            @IN7 = <IN7>;
            for (my $j = 0; $j < scalar @IN7; $j++){
            my $INrow = $IN7[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_5res_ref = $corr_test;}
            }
            close IN7;
        open(IN8, "<"."corr_5res_$fileIDq"."_$sample.txt") or die "could open not input file\n";
            @IN8 = <IN8>;
            for (my $j = 0; $j < scalar @IN8; $j++){
            my $INrow = $IN8[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_5res_query = $corr_test;}
            }
            close IN8;
        open(IN9, "<"."corr_9res_$fileIDr"."_$sample.txt") or die "could open not input file\n";
            @IN9 = <IN9>;
            for (my $j = 0; $j < scalar @IN9; $j++){
            my $INrow = $IN9[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_9res_ref = $corr_test;}
            }
            close IN9;
        open(IN10, "<"."corr_9res_$fileIDq"."_$sample.txt") or die "could open not input file\n";
            @IN10 = <IN10>;
            for (my $j = 0; $j < scalar @IN10; $j++){
            my $INrow = $IN10[$j];
            my @INrow = split (/\s+/, $INrow);
            $atom_number = @INrow[0];
            $corr_test = @INrow[2];
            if($atom_number/4 == $i){$corr_9res_query = $corr_test;}
            }
            close IN10;
        ###############
        #print "$sample\t"."$pos_ref\t"."$res_ref\t"."$res_query\t"."$atomnumber\t"."$atomlabel\t"."$flux_ref\t"."$flux_query\n";
        print OUT "$sample\t"."$pos_ref\t"."$res_ref\t"."$res_query\t"."$atomnumber\t"."$atomlabel\t"."$flux_ref\t"."$flux_query\t"."$corr_1res_ref\t"."$corr_1res_query\t"."$corr_3res_ref\t"."$corr_3res_query\t"."$corr_5res_ref\t"."$corr_5res_query\t"."$corr_9res_ref\t"."$corr_9res_query\n";;
        }
    close IN2;
    close OUT;
}
