...
my $pdbFrame = $mw->Frame();
	...
	my $startFrame = $pdbFrame->Frame();
		my $startLabel = $startFrame->Label(-text=>"start numbering AA's on chain(s) at (use 1 for multiple chains): ");
...
my $stopButton = $mw -> Button(-text => "exit GUI", 
				-command => \&stop
				); # Creates a file button
...
print "MD control file is made (see MDr.ctl)\n";
################################
...
print ctlFile4 "atomicfluct out fluct_$fileIDr"."_$i.txt \@CA,C,O,N&!(:WAT)\n";
...
######################################################################################################
sub reduce { # create reduced PDB files for teLeap
...
print "select input file (e.g. wat_1ubqREDUCED.prmtop)\n";
print "add trajectory file (e.g. prod_1ubqREDUCED_0.nc)\n";
