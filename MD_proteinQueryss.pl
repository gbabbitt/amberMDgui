...
open(SANDER_MIN_AA, ">"."$protein_label"."_min.in") or die "could not open SANDER_MINIMIZATION file\n";
	...
	print SANDER_MIN_AA "maxcyc=3000,\n";
	print SANDER_MIN_AA "ncyc=2000,\n";
...
open(SANDER_HEAT_AA, ">"."$protein_label"."_heat.in") or die "could not open SANDER_HEATING file\n";
	...
	print SANDER_HEAT_AA "temp0=$Temp_Query,\n"; 
	...
	if($method eq "implicit"){print SANDER_HEAT_AA "saltcon = $Salt_Conc,\n";}
...
# Prepare equilibration file for sander
open(SANDER_EQ_AA, ">"."$protein_label"."_eq.in") or die "could not open SANDER_EQ file\n";
	...
	print SANDER_EQ_AA "tempi=$Temp_Query,\n"; 	
	print SANDER_EQ_AA "temp0=$Temp_Query,\n"; 
...
open(SANDER_EQ_AA, ">"."$protein_label"."_rand$jj.in") or die "could not open SANDER_EQ file\n";
	print SANDER_EQ_AA "Equilibration\n"; 
	print SANDER_EQ_AA "&cntrl\n"; 
	print SANDER_EQ_AA "imin=0,\n"; 		
	print SANDER_EQ_AA "ntx=1,\n"; 		
	print SANDER_EQ_AA "irest=0,\n"; 
	print SANDER_EQ_AA "nstlim=$len_rand,\n"; 
	print SANDER_EQ_AA "dt=0.002,\n"; 	
	print SANDER_EQ_AA "ntf=2,\n"; 		
	print SANDER_EQ_AA "ntc=2,\n"; 		
	print SANDER_EQ_AA "tempi=$Temp_Query,\n"; 	
...
open(SANDER_PROD_AA, ">"."$protein_label"."_prod.in") or die "could not open SANDER_PRODUCTION file\n";
	...
	print SANDER_PROD_AA "tempi=$Temp_Query,\n";
	print SANDER_PROD_AA "temp0=$Temp_Query,\n";
...
chomp $pid;
	...
	#print "$chkpid\n";
       	chomp $chkpid;
        #print "check pmemd PID = "."$pid\t"."$chkpid\n"; 
        if ($pid eq $chkpid) {print "\ $seconds"." secs into MD run\t"."rand_$protein_label"."_$jj.out\n";}
	#	if ($pid ne $chkpid) {$pid = ''; $chkpid = '';}
		    #system("tail -f amino_acid_min_$protein_label.out".".out")          
	}
print "\ end MD run $jj\n";
...
chomp $pid;
	...
	#print "$chkpid\n";
       	chomp $chkpid;
        #print "check pmemd PID = "."$pid\t"."$chkpid\n"; 
        if ($pid eq $chkpid) {print "\ $seconds"." secs into MD run\t"."prod_$protein_label"."_$jj.out\n";}
	#	if ($pid ne $chkpid) {$pid = ''; $chkpid = '';}
		    #system("tail -f amino_acid_min_$protein_label.out".".out")          
	}
print "\ end MD run $jj\n";
